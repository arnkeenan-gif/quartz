...
async function loadShopProducts() {
  ...
  // Then try to fetch from Supabase for any dynamic additions
  try {
    ...
    if (res.ok) {
      ...
      if (dbProducts && dbProducts.length > 0) {
        // Merge DB products with local (DB takes priority for matching IDs)
    ...
    // Supabase table not set up yet - local data works fine
